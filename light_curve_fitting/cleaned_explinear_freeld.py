...
def vectorized_model(t, yerr, y=None, mu_duration=None, mu_t0=None, mu_depths=None, PERIOD=None, trend_fixed=None, ld_interpolated=None, ld_fixed=None):
    ...
    if trend_fixed is None:
        ...
        A = numpyro.sample('A', dist.Normal(0, 0.01).expand([num_lcs]))
        tau = numpyro.sample('tau', dist.Normal(1, 0.5).expand([num_lcs]))
    else: 
...
def whitelight_model(t, yerr, y=None, prior_params=None):
    ...
    v = numpyro.sample('v', dist.Normal(0, 0.01))
    A = numpyro.sample('A', dist.Normal(0, 0.01))
    tau = numpyro.sample('tau', dist.Normal(1, 1))
    params = {
        "period": prior_params['period'], "duration": duration, "t0": t0, "b": b,
        "rors": rors, "u": u, 'c': c, 'v': v, 'A': A, 'tau': tau
    }
    lc_model = compute_lc_from_params(params, t)
   ...
   #validate_config(cfg)
    ...
    if not stringcheck:
        ...
        map_params = {'duration': soln['duration'], 't0': soln['t0'], 'rors':soln['rors'], 'period': PERIOD_FIXED, 'b': PRIOR_B,
                      'u': soln['u'], 'c': soln['c'], 'v': soln['v'], 'A': soln['A'], 'tau': soln['tau']}
        ...
        bestfit_params_wl = {'duration': jnp.nanmedian(wl_samples['duration']), 't0': jnp.nanmedian(wl_samples['t0']),
                            ...
                            'c': jnp.nanmedian(wl_samples['c']), 'v': jnp.nanmedian(wl_samples['v']),
                             'A': jnp.nanmedian(wl_samples['A']), 'tau': jnp.nanmedian(wl_samples['tau'])
                            }
    ...
    if need_lowres_analysis:
        ...
        init_params_lr = {
            ...
            'A': jnp.full(num_lcs_lr, bestfit_params_wl['A'][0]), 'tau': jnp.full(num_lcs_lr, bestfit_params_wl['tau'][0])
            }
    
        #soln = optimx.optimize(vectorized_model, start=init_params_lr)(key_mcmc_lr,time_lr,flux_err_lr,
        #    flux_lr, mu_duration=DURATION_BASE, mu_t0=T0_BASE,  mu_depths=DEPTHS_BASE_LR, PERIOD=PERIOD_FIXED,)
        #print(init_params_lr)
        #print(soln)
        ...
        trend_tau_lr = np.array(samples_lr['A'])
        ...
        map_params_lr = {
            ...
            'A': jnp.nanmedian(samples_lr['A'], axis=0),
            'tau': jnp.nanmedian(samples_lr['tau'], axis=0)
        }

        lc_transit_all = jax.vmap(compute_lc_from_params, in_axes=(
            { "period": None, "duration": None, "t0": None, "b": None,
             "rors": 0, "u": 0, 'c': 0, 'v': 0, 'A': 0, 'tau': 0},None,))(map_params_lr, time_lr)
    ...
    if need_lowres_analysis:
        ...
        tau_interp_hr = np.polyval(best_poly_coeffs_A, wl_hr)
    ...
    if need_lowres_analysis and not interpolate_trend:
        init_params_hr = {
            ...
            'A': jnp.array(A_interp_hr),
            'tau': jnp.array(tau_interp_hr),
        ...
        init_params_hr = {
                ...
                'A': jnp.full(num_lcs_hr, bestfit_params_wl['A'][0]),
                'tau': jnp.full(num_lcs_hr, bestfit_params_wl['tau'][0]),
            }
    # --- High-Res MCMC  ---
