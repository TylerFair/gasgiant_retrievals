...
def vectorized_model(t, yerr, y=None, mu_duration=None, mu_t0=None, mu_depths=None, PERIOD=None, trend_fixed=None, ld_interpolated=None, ld_fixed=None):
    ...
    depths = numpyro.sample('depths', dist.TruncatedNormal(mu_depths, 2e-4 * jnp.ones_like(mu_depths), low=0.0, high=1.0).expand([num_lcs]))
    rors = numpyro.deterministic("rors", jnp.sqrt(depths))

    if trend_fixed is None:
        c = numpyro.sample('c', dist.Normal(1, 0.01).expand([num_lcs]))
        v = numpyro.sample('v', dist.Normal(0, 0.01).expand([num_lcs]))
    else: 
        trend_temp = numpyro.deterministic('trend_temp', trend_fixed)
        c = numpyro.deterministic('c', trend_temp[:, 0])
        v = numpyro.deterministic('v', trend_temp[:, 1])
    ...
    y_model = jax.vmap(compute_lc_from_params, in_axes=(
...
def whitelight_model(t, yerr, y=None, prior_params=None):
    ...
    c = numpyro.sample('c', dist.Normal(1, 0.01))

    params = {
        "period": prior_params['period'], "duration": duration, "t0": t0, "b": b,
        "rors": rors, "u": u, 'c': c, 'v': 0.0
    }
    ...
    def single_lc_gp_log_likelihood(gp_params, y, time_array):

        kernel = tinygp.kernels.quasisep.Matern32(
            scale=jnp.exp(gp_params['GP_log_rho']),
            sigma=jnp.exp(gp_params['GP_log_sigma']),
        )

        mean_func = partial(compute_lc_from_params, params) 

        gp = tinygp.GaussianProcess(
            kernel,
            time_array,
            diag=jnp.exp(gp_params['logs2']),
            mean=mean_func,
        )
        return gp.numpyro_dist()


    gp_params = {'logs2': logs2, 'GP_log_sigma': GP_log_sigma, 'GP_log_rho': GP_log_rho}
    gp = single_lc_gp_log_likelihood(gp_params, y, t)

    numpyro.sample('obs', gp, obs=y)
...
def main():
   ...
   #validate_config(cfg)
    ...
    mini_instrument = 'order'+str(order) if instrument == 'NIRISS/SOSS' else 'nrs'+str(nrs)
    ...
    if not stringcheck:
        ...
        prior_params_wl = {
             ...
             'c': 1.0, 
              'logs2': jnp.log(2*jnp.nanmedian(data.wl_flux_err)), 'GP_log_sigma': jnp.log(jnp.nanmedian(data.wl_flux_err)), 'GP_log_rho': jnp.log(1e-1),
            }

                        
        soln = optimx.optimize(whitelight_model, start=prior_params_wl)(key_master, data.wl_time, data.wl_flux_err, y=data.wl_flux, prior_params=prior_params_wl)

        map_params = {'duration': soln['duration'], 't0': soln['t0'], 'rors':soln['rors'], 'period': PERIOD_FIXED, 'b': PRIOR_B,
                      'u': soln['u'], 'c': soln['c'], 'v': 0.0}
        ...
        bestfit_params_wl = {'duration': jnp.nanmedian(wl_samples['duration']), 't0': jnp.nanmedian(wl_samples['t0']),
                            ...
                            'c': jnp.nanmedian(wl_samples['c']), 'v': 0.0,
                            'logs2': jnp.nanmedian(wl_samples['logs2']),
                            'GP_log_sigma': jnp.nanmedian(wl_samples['GP_log_sigma']),
                            'GP_log_rho': jnp.nanmedian(wl_samples['GP_log_rho'])
                            }

        wl_transit_model = compute_lc_from_params(bestfit_params_wl, data.wl_time) 
        ...
        wl_gp = tinygp.GaussianProcess(
            ...
            mean=partial(compute_lc_from_params, bestfit_params_wl),
        )
       ...
       # wl_residual = data.wl_flux - wl_transit_model 
        ...
        plt.fill_between(data.wl_time, mu + np.sqrt(var), mu - np.sqrt(var), color="C0", alpha=0.5)
        plt.plot(data.wl_time, mu, color="C0", lw=2)
        ...
        plt.figure()
        plt.scatter(data.wl_time[~wl_mad_mask], data.wl_flux[~wl_mad_mask]- (mu[~wl_mad_mask] - wl_transit_model[~wl_mad_mask]), c='k')
        plt.plot(data.wl_time, wl_transit_model, c='r')
        ...
        df = pd.DataFrame({'wl_flux': data.wl_flux, 'wl_err': data.wl_flux_err, 'gp_flux': mu, 'gp_err': jnp.sqrt(var), 'transit_model_flux': wl_transit_model, 'whitelight_outlier_mask': wl_mad_mask})
    ...
    if need_lowres_analysis:
        ...
        flux_lr_gpremoved = jnp.array(data.flux_lr[:,~wl_mad_mask]) / jnp.array(trend_flux[~wl_mad_mask] + 1)

      
        assert time_lr.dtype == 'float64', "time_lr should be float64"
        assert flux_lr_gpremoved.dtype == 'float64', "indiv_y_lr should be float64"
        assert flux_err_lr.dtype == 'float64', "yerr_lr should be float64"
        assert len(time_lr.shape) == 1, "t_lr should be 1D array"
        assert len(flux_lr_gpremoved.shape) == 2, "indiv_y_lr should be 2D array (channels, time)"
        assert len(flux_err_lr.shape) == 2, "yerr_lr should be 2D array (channels, time)"
        assert time_lr.shape[0] == flux_lr_gpremoved.shape[1], "t_lr and indiv_y_lr should have same number of time points"
        ...
        soln = optimx.optimize(vectorized_model, start=init_params_lr)(key_mcmc_lr,time_lr,flux_err_lr,
            flux_lr_gpremoved, mu_duration=DURATION_BASE, mu_t0=T0_BASE,  mu_depths=DEPTHS_BASE_LR, PERIOD=PERIOD_FIXED, ld_fixed=ld_fixed_lr)
        
        print("Sampling low-res model using MCMC to find median coefficients...")
    
        samples_lr = get_samples(
            partial(vectorized_model, mu_duration=DURATION_BASE, mu_t0=T0_BASE, mu_depths=DEPTHS_BASE_LR, PERIOD=PERIOD_FIXED),
            key_mcmc_lr, time_lr, flux_err_lr, flux_lr_gpremoved, soln, ld_fixed=ld_fixed_lr)
        ...
        lc_transit_all = jax.vmap(compute_lc_from_params, in_axes=(
            { "period": None, "duration": None, "t0": None, "b": None,
              "rors": 0, "u": 0, 'c': 0, 'v': 0},None,))(map_params_lr, time_lr)

        model_all = lc_transit_all 
        residuals = flux_lr_gpremoved - model_all   
        ...
        n_channels, n_times = flux_lr_gpremoved.shape
        ...
        for ch in range(n_channels):
            ...
            if outlier_mask.any():
                ax = axes[plot_count] 
                ax.scatter(time_lr, flux_lr_gpremoved[ch, :], c="k", label="data", s=10)

                ax.scatter(time_lr[outlier_mask], flux_lr_gpremoved[ch, outlier_mask],
        ...
        flux_lr_gpremoved = flux_lr_gpremoved[:, valid]
        flux_err_lr    = flux_err_lr[:, valid]
       
        print("Plotting low-resolution fits and residuals...")
        plot_map_fits(time_lr, flux_lr_gpremoved, flux_err_lr, data.wavelengths_lr, map_params_lr,
                    {"period": PERIOD_FIXED, 'ld_fixed': ld_fixed_lr},
                    f"{output_dir}/22_{instrument_full_str}_{low_resolution_bins}bins_bestfit.png", ncols=5)
        plot_map_residuals(time_lr, flux_lr_gpremoved, flux_err_lr, data.wavelengths_lr, map_params_lr,
                        {"period": PERIOD_FIXED, 'ld_fixed': ld_fixed_lr},
                        f"{output_dir}/23_{instrument_full_str}_{low_resolution_bins}bins_residual.png", ncols=5)
        ...
        save_results(wl_lr, samples_lr, mini_instrument, f"{output_dir}/{instrument_full_str}_{low_resolution_bins}bins.csv")
        ...
        rms_vals = jax.vmap(calc_rms)(flux_lr_gpremoved)
        ...
        u1_16, u1_median, u1_84 = np.percentile(np.array(samples_lr["u"][:,:,0]), [16, 50, 84], axis=0) # Shape: (n_samples, n_lcs, 2)
        u1_err_low = u1_median - u1_16
        u1_err_high = u1_84 - u1_median
        u1_yerr = np.array([u1_err_low, u1_err_high]) 

        u2_16, u2_median, u2_84 = np.percentile(np.array(samples_lr["u"][:,:,1]), [16, 50, 84], axis=0)
        u2_err_low = u2_median - u2_16
        u2_err_high = u2_84 - u2_median
        u2_yerr = np.array([u2_err_low, u2_err_high])

        plt.figure(figsize=(10, 6)) 
        plt.errorbar(wl_lr, u1_median, yerr=u1_yerr, fmt='o', markersize=4,
                    capsize=3, elinewidth=1, markeredgewidth=1, mfc='w',mec='k', label='u1 Median ± 1σ')
        plt.xlabel('Wavelength (micron)')
        plt.ylabel('u1')
        plt.tight_layout()
        u1_save_path = f'{output_dir}/2optional5_{instrument_full_str}_{low_resolution_bins}bins_u1.png'
        plt.savefig(u1_save_path)
        print(f"Saved u1 plot with uncertainties to {u1_save_path}")
        plt.close() 

        plt.figure(figsize=(10, 6)) 
        plt.errorbar(wl_lr, u2_median, yerr=u2_yerr, fmt='o', markersize=4,
                    capsize=3, elinewidth=1, markeredgewidth=1, mfc='w',mec='k', label='u2 Median ± 1σ')
        plt.xlabel('Wavelength (micron)')
        plt.ylabel('u2')
        plt.tight_layout()
        u2_save_path = f'{output_dir}/2optional6_{instrument_full_str}_{low_resolution_bins}bins_u2.png'
    ...
    n_times = time_hr.shape[0]
    #flux_hr = jnp.array(data.flux_hr[:, ~wl_mad_mask])
    #flux_err_hr = jnp.array(data.flux_err_hr[:, ~wl_mad_mask])
    flux_err_hr = jnp.array(data.flux_err_hr[:, ~wl_mad_mask])
    flux_hr_gpremoved = jnp.array(data.flux_hr[:,~wl_mad_mask]) / jnp.array(trend_flux[~wl_mad_mask] + 1)

    if need_lowres_analysis:
        valid = ~time_mask     
        time_hr       = time_hr[valid]
       # flux_hr = flux_hr[:, valid]
        flux_hr_gpremoved = flux_hr_gpremoved[:, valid]
        flux_err_hr    = flux_err_hr[:, valid]

    assert time_hr.dtype == 'float64', "t_hr should be float64"
    assert flux_hr_gpremoved.dtype == 'float64', "indiv_y_hr should be float64"
    assert flux_err_hr.dtype == 'float64', "yerr_hr should be float64"
    assert len(time_hr.shape) == 1, "t_hr should be 1D array"
    assert len(flux_hr_gpremoved.shape) == 2, "indiv_y_hr should be 2D array (channels, time)"
    assert len(flux_err_hr.shape) == 2, "yerr_hr should be 2D array (channels, time)"
    assert time_hr.shape[0] == flux_hr_gpremoved.shape[1], "t_hr and indiv_y_hr should have same number of time points"
    ...
    samples_hr = get_samples(
        partial(vectorized_model,
                ...
                PERIOD=PERIOD_FIXED),
        key_mcmc_hr, time_hr, flux_err_hr, flux_hr_gpremoved, init_params_hr, 
        trend_fixed=trend_fixed_hr, # Pass fixed trend if calculated
        ld_interpolated=ld_interpolated_hr, # Pass interpolated LD if calculated
    ...
    save_results(wl_hr, samples_hr, mini_instrument, f"{output_dir}/{instrument_full_str}_{high_resolution_bins}bins.csv")
    ...
    rms_vals = jax.vmap(calc_rms)(flux_hr_gpremoved)
