...
class Profile:
    ...
    def set_from_params_dict(self, profile_type, params_dict):
        if profile_type == "isothermal":
            ...
            self.set_parametric(
                ...
                10**params_dict["log_P2"], 10**params_dict["log_P3"], 10**params_dict['log_ref_pressure'])
    ...
    def set_parametric(self, T_set, P1, alpha1, alpha2, P2, P3, ref_pressure):
        '''Parametric model from https://arxiv.org/pdf/0910.1347.pdf'''
        P0 = xp.amin(self.pressures)

        i_set = xp.argmin(xp.absolute(self.pressures - ref_pressure))
        p_set_i = self.pressures[i_set]
        ...
        if ref_pressure >= P3:
            T3 = T_set
            T2 = T3 - xp.power(xp.log(P3/P2)/alpha2, 2.0)
            T1 = T2 + xp.power(xp.log(P1/P2)/alpha2, 2.0)
            T0 = T1 - xp.power(xp.log(P1/P0)/alpha1, 2.0) 
        elif ref_pressure >= P1:
            T2 = T_set - xp.power(xp.log(p_set_i/P2)/alpha2, 2.0)
            T1 = T2 + xp.power(xp.log(P1/P2)/alpha2, 2.0)
            T3 = T2 + xp.power(xp.log(P3/P2)/alpha2, 2.0)
            T0 = T1 - xp.power(xp.log(P1/P0)/alpha1, 2.0)
        elif ref_pressure < P1:
            T0 = T_set - xp.power(xp.log(p_set_i/P0)/alpha1, 2.0)
            T1 = T0 + xp.power(xp.log(P1/P0)/alpha1, 2.0)
            T2 = T1 - xp.power(xp.log(P1/P2)/alpha2, 2.0)
            T3 = T2 + xp.power(xp.log(P3/P2)/alpha2, 2.0)

        assert T0 is not None, "T0 was not set!"
        assert T2 is not None, "T2 was not set!"
        assert T3 is not None, "T3 was not set!"
        mask_1 = self.pressures < P1
        mask_2 = (self.pressures >= P1) & (self.pressures < P3)
        mask_3 = self.pressures >= P3


        self.temperatures = xp.zeros(len(self.pressures))

        self.temperatures[mask_1] = T0 + xp.power(xp.log(self.pressures[mask_1]/P0)/alpha1, 2.0)
        self.temperatures[mask_2] = T2 + xp.power(xp.log(self.pressures[mask_2]/P2)/alpha2, 2.0)
        self.temperatures[mask_3] = T3 
